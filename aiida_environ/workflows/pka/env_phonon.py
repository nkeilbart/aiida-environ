...
class EnvPhononWorkChain(WorkChain, ProtocolMixin):
    ...
    def define(cls, spec):
        ...
        spec.expose_inputs(
            EnvPwBaseWorkChain, namespace='scf',
            namespace_options={
                'required': True,
                'help': ('Inputs for the `EnvPwBaseWorkChain` that will be used to run the electric enthalpy scfs.')
            },
        ...
        spec.input(
            'settings.sleep_submission_time', valid_type=(int, float), non_db=True, default=0.1,
        ...
        spec.exit_code(
            ...
            message='At least one sub process did not finish successfully.'
        )
        spec.exit_code(
    ...
    def get_builder_from_protocol(
        ...
        **kwargs
        ...
        scf = EnvPwBaseWorkChain.get_builder_from_protocol(
            *args, 
    ...
    def run_base_supercell(self):
        ...
        inputs = AttributeDict(self.exposed_inputs(EnvPwBaseWorkChain, namespace='scf'))
    ...
    def run_forces(self):
        ...
        for _ in self.ctx.supercells[:n_base_parallel]:
            ...
            parameters['ELECTRONS']['startingwfc'] = 'atomic+random'
            inputs.pw.parameters = orm.Dict(parameters)
            ...
            self.report(f'submitting `EnvPwBaseWorkChain` <PK={future.pk}> with supercell n.o {num}')
        ...
        for label, workchain in self.ctx.items():
            if label.startswith(self._RUN_PREFIX):
                if workchain.is_finished_ok:
                    ...
                    self.report(
                        f'EnvPwBaseWorkChain with <PK={workchain.pk}> failed '
                        f'with exit status {workchain.exit_status}'
                    )
                    failed_runs.append(workchain.pk)

        if failed_runs:
            self.report('one or more workchains did not finish successfully')
