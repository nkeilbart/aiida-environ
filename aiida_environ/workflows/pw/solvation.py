...
class PwSolvationWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input('environ_vacuum', valid_type=Dict, required=False, 
    ...
    def setup(self):
        ...
        self.ctx.vacuum_inputs.pw.environ_parameters['ENVIRON'].setdefault('environ_thr', 1e-1)
        self.ctx.vacuum_inputs.pw.environ_parameters['ENVIRON'].setdefault('environ_type', 'vacuum')
        self.ctx.vacuum_inputs.pw.environ_parameters['ENVIRON'].setdefault('environ_restart', False)
        ...
        self.ctx.solution_inputs.pw.environ_parameters['ENVIRON'].setdefault('verbose', 0)
        self.ctx.solution_inputs.pw.environ_parameters['ENVIRON'].setdefault('environ_thr', 1e-1)
        self.ctx.solution_inputs.pw.environ_parameters['ENVIRON'].setdefault('environ_type', 'water')
        self.ctx.solution_inputs.pw.environ_parameters['ENVIRON'].setdefault('environ_restart', False)
        self.ctx.solution_inputs.pw.environ_parameters['ENVIRON'].setdefault('env_electrostatic', True)

        self.ctx.solution_inputs.pw.environ_parameters.setdefault('ELECTROSTATIC', {})
        self.ctx.solution_inputs.pw.environ_parameters['ELECTROSTATIC'].setdefault('solver', 'cg')
        self.ctx.solution_inputs.pw.environ_parameters['ELECTROSTATIC'].setdefault('auxiliary', 'none')
