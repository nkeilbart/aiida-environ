...
from aiida.orm import List, Float, Int, Str, Dict
from aiida.engine import ToContext, submit, WorkChain
...
class ParameterizationWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input('expt_energy', valid_type=List,
    ...
    def run_vacuum(self):
        ...
        for i, structure_pk in enumerate(self.inputs.structure_pks):
            ...
            inputs.pw.pseudos = get_pseudos_from_structure(struct, self.inputs.pseudo_label.value)
            ...
            self.report(f'launching vacuum EnvPwBaseWorkChain<{future.pk}> w/ Structure<{struct.pk}>')
    ...
    def run_solution(self):
        ...
        for i, structure_pk in enumerate(self.inputs.structure_pks):
            ...
            self.report(f'launching solution_0 EnvPwBaseWorkChain<{future.pk}> w/ Structure<{struct.pk}>')
            calculations[key] = future
        ...
        for i, structure_pk in enumerate(self.inputs.structure_pks):
            ...
            self.report(f'launching solution_1 EnvPwBaseWorkChain<{future.pk}> w/ Structure<{struct.pk}>')
            calculations[key] = future
    ...
    def post_processing(self):
        self.ctx.results = calc_partial(
            Int(self.ctx.nstruct), Float(self.ctx.delta), self.inputs.expt_energy, Dict(dict=self.ctx.calculations))
