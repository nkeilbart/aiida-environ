...
def generate_structures_n(size: Tuple[int, int], n: int):
...
def perm_to_coords(perm: np.array):
    coords = []
...
def intranslatable_coords(coords: List):
    itcoords = {}
...
def generate_structures(size, structure, vacancies):
    ...
    if n > 3:
        for i in range(2, n-1):
            inner_perms = generate_structures_n(size, i)
    ...
    # TODO: use solution for adsorbate_calc
    struct_list = []
